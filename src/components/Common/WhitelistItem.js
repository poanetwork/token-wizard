...
export class WhitelistItem extends React.Component {
  ...
  render() {
    ...
    return (
      <div
        className={classNames('white-list-item-container', {
      ...
      >
        ...
        <div className="white-list-item-empty">
          {!stored ? (
            <a onClick={() => this.removeItem()} className="remove">
              <span className="item-remove" />
            </a>
          ) : null}
          {duplicated && !stored ? (
            <a className="swal2-icon swal2-info warning-logo" data-tip="React-tooltip">
              !
            </a>
          ) : null}
        </div>
