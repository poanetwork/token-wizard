...
export const ContributeForm = inject('contributeStore', 'tokenStore')(
  observer(
    ({ contributeStore, tokenStore, handleSubmit, pristine, invalid, isEnded, isFinalized, isSoldOut, ...props }) => {
      const { decimals } = tokenStore
      const { contributeThrough, updateContributeThrough, web3Available } = props

      const buttonDisabled = pristine || invalid || isEnded || isFinalized || isSoldOut
